        ...
        {Menus.map((menu, idx) => {
          ...
          return (
            <Link
              key={idx}
              to={menu.Path}
              className={`group relative flex flex-col items-center transition-all duration-300 ease-in-out ${isActive
            ...
            >
              {menu.title}
              <span
                className={` h-[3px] rounded transition-all duration-300 ${isActive
                  ? "w-full bg-blue-600"
      ...
      <div className="text-base text-blue-700 font-medium">
        <div className="flex items-center space-x-4 px-4 py-2 bg-white rounded-lg shadow-inner">
          ...
          <div className="text-black">
            <span>{currentTime.toISOString().split("T")[0].replace(/-/g, "/")}</span>
          </div>
        </div>
      </div>
    </header>
